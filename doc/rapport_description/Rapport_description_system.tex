...
L'entreprise travaille avec principalement avec des entreprises qui nécessitent des relevés 3D d'un environnement en particulier. Ces dernières sont notamment dans le domaine du BTP "Bâtiment et Travaux Publics", ou encore l'industrie. En effet, lorsqu'une entreprise a besoin de rénover, ou d'apporter du changement en général sur un lieu, il faut pour cela planifier les travaux. Cette planification peut prendre du temps, et nécessite d'établir des plans. \newline

Il existe plusieurs types de plans : physique avec des plans sur papiers, ou numérique. L'évolution technologique fait que de plus en plus d'entreprises peuvent avoir besoin de plans numériques. En effet, ces derniers permettent de pouvoir y apporter des modifications sans devoir réimprimer tous les plans qui étaient déjà imprimés, faciliter les démarches administratives, avoir une meilleure vision sur le résultat final, grâce à la modélisation 3D. \newline

De plus, dans les plans numériques, la plupart des entreprises proposent de se servir d'un nuage de points en 3D, afin de réaliser une reconstruction CAO (Conception assistée par ordinateur), c'est-à-dire de convertir le nuage de points, en modélisations géométriques et ainsi avoir des objets "virtuels" que l'on pourra traiter. Le problème de ce processus est que le coût de temps est très important, et ralenti fortement le délai de rénovation d'un lieu. \newline

Taglabs propose donc d'éviter ce processus sur un nuage de points complet. En effet, le but de l'entreprise est d'extaire toutes les informations nécessaires pré-traitement, afin d'accélérer la modélisation des éléments incontournables.
...
Nous pouvons observer que notre avancement est en bonne voie. Etant à la moitié du projet, nous avons réalisé la moité des tâches. Il faut aussi noter que les composants sont amenés à évoluer, avec le feedback du client à nos fins d'itérations. Par exemple, la tâche du point picking, ou encore le toon mapping sont des composants qui ont été soulevé suite à nos réunions et discussions avec le client et notre tuteur académique.
...
La structure du code source de CloudCompare est bien organisée pour éviter d'avoir du code éparpillé partout dans le projet original. Nous avons juste à insérer notre code et développer notre propre plugin dans le dossier plugins.
...
Dans notre cas, nous avons par exemple ajouté des méthodes afin d'ajouter un signal sur un bouton, pour activer un point picking, c'est-à-dire pour activer le mode de sélection de points dans le nuage.

\subsubsection{Gestion des actions sur l'interface}

Les actions utilisateurs sont également gérées grâce à Qt au travers de la mécanique des signaux et des slots. Chaque événement, tel que l'ajout d'un nuage de points, est représenté par un slot et un signal.
